...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    db.collection('items').where('item_id', '==', to.params.item_id).get().then(queryShapshot => {
  ...
  methods: {
    fetchData () {
      db.collection('items').where('item_id', '==', this.$route.params.item_id).get().then(queryShapshot=>{
        queryShapshot.forEach(doc => {
